...
function emptyInputSignup($phonenumber, $name, $address){
  $result;

  if(empty($phonenumber) || empty($name) ||empty($address)){
      $result = true;
  }
  else {
    $result = false;
...
function loginUser($conn, $phonenumber, $connlogin, $user ){
  ...
  if ($numberExists === false) {
    ...
    $sql = "INSERT INTO users (usersPhonenumber, usersName) VALUES (?, ?);";
    $stmt = mysqli_stmt_init($connlogin);
